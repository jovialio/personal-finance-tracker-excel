...
- **Expense Trends & Expense Charts** — a visual spending history. *Expense Trends* is a formula-driven table that breaks each active expense category into its own monthly column (off Monthly Summary); *Expense Charts* carries a total-monthly-expenses line plus one small trend chart per category, so each category’s trajectory over time is visible at a glance. Both extend automatically as you add Entries.

Tabs are colour-coded — **green = you edit** (Entries, Budgets, Categories), **blue = automatic & locked** (Dashboard, Monthly Summary, Expense Trends, Expense Charts), **grey = the guide** (Instructions). Formula sheets are protected with no password (`Review → Unprotect Sheet` to change). It ships with fictional sample data (2008–2025) so every formula and chart shows a working result — clear the Entries and Budgets rows (keep the headers) to make it yours. The **Instructions** tab is the full manual.
