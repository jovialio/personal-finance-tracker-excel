...
- **Allocation: unmapped holdings now count toward Total assets.** In v1.15, a holding whose `Prices` category was not yet in the `Category → Asset Class` map was labelled `UNMAPPED` in the detail table but fell outside the six asset-class buckets — so `Allocation!B5` (Total assets) and the 100% check summed only the mapped buckets and silently dropped it from the total-assets denominator, even though the weights still read 100%. Total assets now adds an explicit `SUMIFS(…,"UNMAPPED")` term, so the total covers **every** holding regardless of mapping, and a new data-quality line on the tab warns — with a count and dollar value — whenever any holding is unmapped, pointing you to the category map to fix it. Investible and Risk stay bucket-defined lenses (an unmapped holding shows a Total % but blank lens %, matching the per-row formulas) until you map its category, at which point it flows into all three lenses. The shipped sample is fully mapped, so no displayed figure changes. The README now also documents the Allocation tab.
