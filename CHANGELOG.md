...
- **Instructions: added a "Starting fresh" section.** Spells out how to clear the sample data (clear the contents of the Entries and Budgets rows, keeping the header and table), enter your own data from row 2, date each entry to any day in its month, and leave the auto columns alone.

## v1.16

- **New companion tool: Monthly Budget Tracker.** Added `Monthly Budget Tracker.xlsx`, a standalone monthly cash-flow budgeting workbook that is independent of the portfolio tracker (no existing tab is changed). One **Entries** ledger (Date · Category · Type · Tier · Amount · Note) drives everything: a **Categories** master list whose expense categories auto-expand into the breakdown with no formula editing (set `Active = N` to retire one), effective-dated **Budgets** (add a dated row to revise a budget; history stays accurate), an auto-extending **Monthly Summary** (monthly totals, per-category budget-vs-actual, 12-month rolling averages and cumulative cash via `SUMIFS`, pre-filled years ahead so each month fills in on its own), and a **Dashboard** whose headline figures are trailing-12-month averages — average monthly savings, true lifestyle and core/min, each averaged over the **active (non-zero) months within the last 12 calendar months** — alongside charts for cumulative cash, income vs spend, rolling savings ratio, expenditure by tier and budget-vs-actual by category. Tabs are colour-coded (green = edit, blue = automatic & locked, grey = guide) and formula sheets are protected with no password. Ships with fictional 2008–2025 sample data so every formula and chart shows a working result; clear the Entries and Budgets rows to make it yours.
- **Necessity tiers (Core / Over & above / One-off / Reimbursable / Investment).** Each expense carries a Tier alongside its Category, so a single category can split between essential and trimmable spend (e.g. Transport = Core bus/MRT + Over & above cab fares). From it the Monthly Summary derives **minimum-core lifestyle**, **true lifestyle** (Core + Over & above) and **net-of-reimbursable**, and the Dashboard gains "Expenditure by tier" and "Min vs True lifestyle vs Actual" charts. Reproduces (and extends) the original sheet's core / lifestyle / actual expenditure views.
- **Docs & repo hygiene.** README gains a "Companion tool — Monthly Budget Tracker" section; `.gitignore` now excludes personal data and per-user config so real financial data is never committed.

## v1.15.2

- **Fixed: a fully-sold position could keep showing phantom units.** The `Signed Units` helper on Transactions was computed as *action code × quantity* — correct for Buy (`1`) and Sell (`−1`), but it treats Dividend (action `2`) as a **2× multiplier**, so a dividend row carrying any value in the `Units Purchased or Sold` column injected twice that many phantom units into the position. A full sell then couldn't bring the holding to zero, and the ticker lingered as "held" everywhere the workbook derives holdings from units: Shares `Units Held`, the Allocation holdings-detail table, and the Prices `TICKER SYNC` panel (staying "OK - in Prices" instead of flipping to "NOT HELD - review"). `Signed Units` is now `IF(action=1, units, IF(action=-1, -units, 0))`, so dividends and fees never affect the unit count no matter what's typed in that column. Buys and sells are unchanged, so no sample figure moves — but a genuinely exited position now correctly drops off Shares, the Allocation detail and the sync panel. (Genuine scrip/DRIP shares should still be recorded as a separate Buy.)
- **Start Here: clarified how to remove a sold-out ticker from Prices.** When the `TICKER SYNC` panel flags a row "NOT HELD - review," delete the whole table row (right-click the row number → Delete, or right-click a cell → Delete → Table Rows) rather than clearing the cells — a cleared-but-not-deleted row leaves a blank entry that keeps reading "NOT HELD - review" and adds 1 to the Stale count. No calculation impact either way; this just removes the misleading line.

## v1.15.1

- **Allocation: Total assets no longer depends on classification, and the data-quality check catches blank categories.** In v1.15 a holding whose `Prices` Category wasn't in the `Category → Asset Class` map fell outside the six asset-class buckets, so `Allocation!B5` (Total assets) silently dropped it while the weights still read 100%. Total assets is now summed directly from the holdings, `SUM(H30:H279)+SUM(C22:C26)`, so it covers **every** holding and the `Other assets` block no matter how any category resolves — including a *blank* Category, which the earlier literal-`UNMAPPED` approach missed because a blank category resolves to a blank class rather than the word `UNMAPPED`. A data-quality line on the tab now warns, with the dollar amount, whenever the classified buckets don't add up to that total — catching blank, unknown, or `UNMAPPED` categories alike — and the Prices `TICKER SYNC` panel now reminds you that every held ticker needs a Category. Investible and Risk stay bucket-defined lenses until you classify the holding, at which point it flows into all three. The shipped sample is fully classified, so no displayed figure changes. The README now also documents the Allocation tab.
